...
describe('isToolAvailable', () => {
    const b2c = { businessModel: 'b2c', marketplaceEnabled: false };
    const b2b = { businessModel: 'b2b', marketplaceEnabled: false };
    const b2cMp = { businessModel: 'b2c', marketplaceEnabled: true };
    const b2bMp = { businessModel: 'b2b', marketplaceEnabled: true };
    ...
    it('combines model and marketplace constraints', () => {
        const avail = { availability: { models: ['b2b'], marketplace: true } };
...
describe('registry filtering by business model', () => {
    ...
    afterEach(() => {
        if (realModel === undefined) {
            ...
            process.env.SPRYKER_BUSINESS_MODEL = realModel;
        }
        if (realMp === undefined) {
            delete process.env.SPRYKER_MARKETPLACE_ENABLED;
        }
        else {
            process.env.SPRYKER_MARKETPLACE_ENABLED = realMp;
        }
    });
...
//# sourceMappingURL=business-model.test.js.map