...
describe('createWishlistTool', () => {
    it('should create a wishlist', async () => {
        ...
        expect(mockApiService.post).toHaveBeenCalledWith('wishlists', { data: { type: 'wishlists', attributes: { name: 'Holiday' } } }, 'auth');
...
describe('addToWishlistTool', () => {
    it('should add an item to a wishlist', async () => {
        ...
        expect(mockApiService.post).toHaveBeenCalledWith('wishlists/w1/wishlist-items', { data: { type: 'wishlist-items', attributes: { sku: 'SKU-1' } } }, 'auth');
        expect(JSON.parse(result.content[0].text).success).toBe(true);
    });
    it('should handle ApiError', async () => {
...
//# sourceMappingURL=wishlists.test.js.map