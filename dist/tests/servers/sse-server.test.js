...
describe('SSEMCPServer', () => {
    ...
    describe('constructor', () => {
        ...
        test('should use default options when not provided', () => {
            const server = new SSEMCPServer({ transport: 'sse' }, mockHandlerRegistrar);
    ...
    describe('start', () => {
        ...
        test('should handle HTTP server startup error', async () => {
            ...
            expect(mockLogger.error).toHaveBeenCalledWith('Failed to start SSE server:', expect.any(Error));
        });
    ...
    describe('shutdown', () => {
        ...
        test('should handle shutdown error gracefully', async () => {
            ...
            await sseServer.shutdown();
            expect(mockLogger.error).toHaveBeenCalledWith('Error during SSE server shutdown:', expect.any(Error));
        });
    ...
    describe('configuration options', () => {
        test('should use provided port option', () => {
            ...
            const server = new SSEMCPServer({ transport: 'sse', httpPort: customPort }, mockHandlerRegistrar);
            expect(server).toBeDefined();
        ...
        test('should use provided host option', () => {
            ...
            const server = new SSEMCPServer({ transport: 'sse', httpHost: customHost }, mockHandlerRegistrar);
            expect(server).toBeDefined();
        });
        test('should use provided endpoint option', () => {
            const customEndpoint = '/custom-sse';
            const server = new SSEMCPServer({ transport: 'sse', httpEndpoint: customEndpoint }, mockHandlerRegistrar);
            expect(server).toBeDefined();
        });
    ...
    describe('SSE-specific functionality', () => {
        test('should handle SSE transport configuration', () => {
            const server = new SSEMCPServer({ transport: 'sse', httpPort: 3002 }, mockHandlerRegistrar);
            expect(server).toBeDefined();
        });
        test('should support different SSE endpoint paths', () => {
            const endpoints = ['/sse', '/events', '/stream', '/mcp-sse'];
            endpoints.forEach(endpoint => {
                const server = new SSEMCPServer({ transport: 'sse', httpEndpoint: endpoint }, mockHandlerRegistrar);
                expect(server).toBeDefined();
            });
        });
    });
    describe('HTTP request handling', () => {
        test('should handle OPTIONS requests', async () => {
            ...
            await sseServer.handleHttpRequest(mockReq, mockRes);
    ...
    describe('HTTP server initialization error handling', () => {
        test('should handle response error in error handler', async () => {
            ...
            const server = new SSEMCPServer(options, {});
            await server.start();
            ...
            jest.spyOn(server, 'handleHttpRequest').mockRejectedValue(new Error('Request handler error'));
