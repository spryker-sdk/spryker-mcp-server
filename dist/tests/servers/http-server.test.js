...
describe('HttpMCPServer', () => {
    let httpServer;
    let mockLogger;
    let mockValidateEnvironment;
    let mockHandlerRegistrar;
    let mockHttpServer;
    beforeEach(() => {
        jest.clearAllMocks();
        mockLogger = logger;
    ...
    describe('constructor', () => {
        ...
        test('should use default options when not provided', () => {
            const server = new HttpMCPServer({ transport: 'http' }, mockHandlerRegistrar);
    ...
    describe('start', () => {
        ...
        test('should handle HTTP server startup error', async () => {
            ...
            errorMockServer.listen.mockImplementation((port, host, callback) => {
            ...
            let errorHandler;
            errorMockServer.on.mockImplementation((event, handler) => {
                if (event === 'error') {
            ...
            expect(mockLogger.error).toHaveBeenCalledWith('Failed to start HTTP server:', expect.any(Error));
        });
    ...
    describe('shutdown', () => {
        ...
        test('should handle shutdown error gracefully', async () => {
            ...
            await httpServer.shutdown();
            expect(mockLogger.error).toHaveBeenCalledWith('Error during HTTP server shutdown:', expect.any(Error));
        });
    ...
    describe('configuration options', () => {
        test('should use provided port option', () => {
            ...
            const server = new HttpMCPServer({ transport: 'http', httpPort: customPort }, mockHandlerRegistrar);
            expect(server).toBeDefined();
        ...
        test('should use provided host option', () => {
            ...
            const server = new HttpMCPServer({ transport: 'http', httpHost: customHost }, mockHandlerRegistrar);
            expect(server).toBeDefined();
        });
        test('should use provided endpoint option', () => {
            const customEndpoint = '/custom-endpoint';
            const server = new HttpMCPServer({ transport: 'http', httpEndpoint: customEndpoint }, mockHandlerRegistrar);
            expect(server).toBeDefined();
        });
    ...
    describe('HTTP request processing', () => {
        ...
        test('should handle POST request to MCP endpoint with valid JSON', async () => {
            ...
            httpServer.handleMCPRequest = mockHandleMCPRequest;
            await httpServer.handleHttpRequest(mockReq, mockRes);
        ...
        test('should handle error response writing failure', async () => {
            ...
            await expect(httpServer.handleHttpRequest(mockReq, mockRes)).rejects.toThrow('Response write error');
            expect(mockLogger.error).toHaveBeenCalledWith('Request handling error:', expect.any(Error));
        });
        test('should handle HTTP request handler error with proper error catching', async () => {
            ...
            const errorHandler = async (req, res) => {
                try {
                    await httpServer.handleHttpRequest(req, res);
                }
                catch (error) {
            ...
            const originalHandleHttpRequest = httpServer.handleHttpRequest;
            httpServer.handleHttpRequest = jest.fn().mockRejectedValue(new Error('Handler error'));
            ...
            httpServer.handleHttpRequest = originalHandleHttpRequest;
        });
    ...
    describe('handleMCPRequest - direct coverage tests', () => {
        ...
        test('should handle initialization request with proper transport creation', async () => {
            ...
            await httpServer.handleMCPRequest(mockReq, mockRes, message);
        ...
        test('should reuse existing transport for session', async () => {
            ...
            httpServer.transports.set('test-session', mockTransport);
            const mockReq = {
        ...
        test('should modify Accept header for existing session', async () => {
            ...
            httpServer.transports.set('session-123', mockTransport);
            const mockReq = {
                headers: {
        ...
        test('should handle transport handleRequest error', async () => {
            ...
            httpServer.transports.set('error-session', mockTransport);
            const mockReq = {
        ...
        test('should not modify Accept header when text/event-stream already present', async () => {
            ...
            httpServer.transports.set('session-456', mockTransport);
            const mockReq = {
                headers: {
