...
function validateNodeVersion() {
    ...
    if (majorVersion < 18) {
        throw new ValidationError(`Node.js version ${nodeVersion} is not supported. Minimum required version is 18.0.0.`, 'INVALID_NODE_VERSION');
    }
...
function validateEnvironmentVariables() {
    ...
    if (requiredVars.length > 0) {
        throw new ValidationError(`Missing required environment variables: ${requiredVars.join(', ')}`, 'MISSING_ENV_VARS');
    }
...
function validateServerConfig() {
    ...
    if (config.api.timeout <= 0) {
        throw new ValidationError('API timeout must be greater than 0', 'INVALID_API_TIMEOUT');
    }
    // Validate retry configuration
    if (config.api.retryAttempts < 0) {
        throw new ValidationError('Retry attempts must be non-negative', 'INVALID_RETRY_ATTEMPTS');
    }
    if (config.api.retryDelay <= 0) {
        throw new ValidationError('Retry delay must be greater than 0', 'INVALID_RETRY_DELAY');
    }
    // Validate rate limiting
    if (config.rateLimit.windowMs <= 0) {
        throw new ValidationError('Rate limit window must be greater than 0', 'INVALID_RATE_LIMIT_WINDOW');
    }
    if (config.rateLimit.maxRequests <= 0) {
        throw new ValidationError('Rate limit max requests must be greater than 0', 'INVALID_RATE_LIMIT_MAX');
    }
...
export async function validateEnvironment() {
    ...
    try {
        ...
        throw new ValidationError('Environment validation failed due to unexpected error', 'VALIDATION_ERROR');
    }
}
/**
...
export function validateToolArguments(args, schema, toolName) {
    try {
        ...
        throw new ValidationError(`Invalid arguments for tool ${toolName}: ${error instanceof Error ? error.message : 'Unknown error'}`, 'INVALID_TOOL_ARGUMENTS');
    }
}
