...
export class SSEMCPServer {
    ...
    async start() {
        ...
        try {
            ...
            logger.error('Failed to start SSE server:', error instanceof Error ? error : new Error(String(error)));
            throw error;
        }
    }
            ...
            if (url.pathname === this.options.httpEndpoint) {
                if (req.method === 'GET') {
                    ...
                    try {
                        ...
                        logger.error('SSE connection failed:', sseError instanceof Error ? sseError : new Error(String(sseError)));
                        if (!res.headersSent) {
                            res.writeHead(500, { 'Content-Type': 'application/json' });
    ...
    async shutdown() {
        ...
        try {
            ...
            logger.error('Error during SSE server shutdown:', error instanceof Error ? error : new Error(String(error)));
        }
    }
}
