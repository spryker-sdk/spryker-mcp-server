...
async function getShoppingLists(args) {
    ...
    try {
        ...
        logger.error('Get shopping lists failed', error);
...
export const getShoppingListsTool = {
    ...
    inputSchema: z.toJSONSchema(GetShoppingListsSchema),
    handler: async (args) => getShoppingLists(GetShoppingListsSchema.parse(args)),
};
...
async function createShoppingList(args) {
    const apiService = SprykerApiService.getInstance();
    try {
        logger.info('Creating shopping list', { name: args.name });
        const response = await apiService.post('shopping-lists', { data: { type: 'shopping-lists', attributes: { name: args.name } } }, args.token);
        return {
            content: [{
                    type: 'text',
                    text: JSON.stringify({
        ...
        logger.error('Create shopping list failed', error);
...
export const createShoppingListTool = {
    ...
    availability: B2B_AVAILABILITY,
    inputSchema: z.toJSONSchema(CreateShoppingListSchema),
    handler: async (args) => createShoppingList(CreateShoppingListSchema.parse(args)),
};
...
async function addToShoppingList(args) {
    const apiService = SprykerApiService.getInstance();
    try {
        logger.info('Adding item to shopping list', { shoppingListUuid: args.shoppingListUuid, sku: args.sku });
        const response = await apiService.post(`shopping-lists/${args.shoppingListUuid}/shopping-list-items`, { data: { type: 'shopping-list-items', attributes: { sku: args.sku, quantity: args.quantity } } }, args.token);
        return {
            content: [{
                    type: 'text',
                    text: JSON.stringify({
        ...
        logger.error('Add to shopping list failed', error);
        return errorResponse('add item to shopping list', error, { shoppingListUuid: args.shoppingListUuid, sku: args.sku });
    }
}
export const addToShoppingListTool = {
    name: 'add-to-shopping-list',
    description: 'Add a concrete product to a B2B shopping list.',
    availability: B2B_AVAILABILITY,
    inputSchema: z.toJSONSchema(AddToShoppingListSchema),
    handler: async (args) => addToShoppingList(AddToShoppingListSchema.parse(args)),
};
...
async function shoppingListToCart(args) {
    const apiService = SprykerApiService.getInstance();
    try {
        logger.info('Moving shopping list items to cart', { shoppingListUuid: args.shoppingListUuid, cartId: args.cartId });
        const listResponse = await apiService.get(`shopping-lists/${args.shoppingListUuid}?include=shopping-list-items`, args.token);
        ...
        for (const item of items) {
            ...
            try {
                await apiService.post(`carts/${args.cartId}/items`, { data: { type: 'items', attributes: { sku, quantity: item.attributes?.quantity ?? 1 } } }, args.token);
                added.push(sku);
            }
            catch (itemError) {
        ...
        logger.error('Shopping list to cart failed', error);
        return errorResponse('move shopping list items to cart', error, { shoppingListUuid: args.shoppingListUuid, cartId: args.cartId });
    }
}
export const shoppingListToCartTool = {
    name: 'shopping-list-to-cart',
    description: 'Move all items from a B2B shopping list into a cart.',
    availability: B2B_AVAILABILITY,
    inputSchema: z.toJSONSchema(ShoppingListToCartSchema),
    handler: async (args) => shoppingListToCart(ShoppingListToCartSchema.parse(args)),
};
//# sourceMappingURL=b2b-shopping-lists.js.map