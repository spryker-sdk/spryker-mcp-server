...
async function getConcreteProduct(args) {
    const apiService = SprykerApiService.getInstance();
    try {
        logger.info('Retrieving concrete product details', { sku: args.sku });
        const response = await apiService.get(`concrete-products/${args.sku}?include=concrete-product-image-sets,concrete-product-availabilities,concrete-product-prices`);
        ...
        logger.error('Get concrete product failed', error);
        return {
            content: [{
                    type: 'text',
                    text: JSON.stringify({
...
export const getConcreteProductTool = {
