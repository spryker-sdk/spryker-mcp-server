...
async function getCheckoutData(args) {
    ...
    try {
        ...
        const response = await apiService.post('checkout-data?include=payment-methods,shipments,shipment-methods,addresses', {
            data: {
                type: 'checkout-data',
                attributes: {
                    idCart: args.cartId,
        ...
        logger.error('Failed to retrieve checkout data', error);
        return {
            content: [{
                    type: 'text',
                    text: JSON.stringify({
...
//# sourceMappingURL=get-checkout-data.js.map