...
} from '../../src/tools/b2b-shopping-lists';
...
describe('createShoppingListTool', () => {
  it('creates a shopping list', async () => {
    ...
    expect(mockApiService.post).toHaveBeenCalledWith(
      'shopping-lists',
      { data: { type: 'shopping-lists', attributes: { name: 'Office' } } },
      'auth'
    );
...
describe('addToShoppingListTool', () => {
  it('adds an item', async () => {
    ...
    expect(mockApiService.post).toHaveBeenCalledWith(
      'shopping-lists/sl-uuid/shopping-list-items',
      { data: { type: 'shopping-list-items', attributes: { sku: 'SKU-1', quantity: 2 } } },
      'auth'
    );
    expect(JSON.parse(result.content[0]!.text).success).toBe(true);
  });
...
describe('shoppingListToCartTool', () => {
  it('moves all items into the cart', async () => {
    ...
    expect(mockApiService.post).toHaveBeenCalledWith(
      'carts/c1/items',
      { data: { type: 'items', attributes: { sku: 'SKU-1', quantity: 3 } } },
      'auth'
    );
    const r = JSON.parse(result.content[0]!.text);
